...
@app.route('/distribute_prizes/<int:contest_id>', methods=['POST'])
@token_required
def distribute_prizes(current_user_email, contest_id):
    try:
        cursor = db.cursor(dictionary=True)

        # 1. Get contest prize pool
        cursor.execute("SELECT prize_pool FROM contests WHERE id = %s", (contest_id,))
        contest = cursor.fetchone()
        if not contest:
            return jsonify({"error": "Contest not found"}), 404
        prize_pool = contest['prize_pool']

        # 2. Get all teams joined in this contest with their total points
        cursor.execute("""
            SELECT e.user_id, e.id AS entry_id, t.id AS team_id, t.total_points
            FROM entries e
            JOIN teams t ON e.team_id = t.id
            WHERE e.contest_id = %s
            ORDER BY t.total_points DESC
        ...
        if not entries:
            return jsonify({"message": "No entries found for this contest."}), 200

        # 3. Assign ranks
        for i, entry in enumerate(entries, start=1):
            entry['rank'] = i

        # 4. Fetch prize distribution percentages
        cursor.execute("SELECT rank, percentage FROM prize_distributions WHERE contest_id = %s", (contest_id,))
        prize_map = {row['rank']: row['percentage'] for row in cursor.fetchall()}
        ...
        for entry in entries:
            ...
            prize = round(prize_pool * prize_map.get(rank, 0) / 100, 2)

            if prize > 0:
                # 5a. Update wallet
                cursor.execute("UPDATE wallets SET balance = balance + %s WHERE user_id = %s", (prize, user_id))

                # 5b. Log transaction
                cursor.execute("""
                    INSERT INTO transactions (user_id, amount, type, description)
                    VALUES (%s, %s, 'credit', %s)
                """, (user_id, prize, f"Prize for contest ID {contest_id}"))

                # 5c. Update entry with prize info (add 'prize' column if not there)
                cursor.execute("UPDATE entries SET prize = %s WHERE id = %s", (prize, entry_id))

                # 5d. Send notification
                message = f"🎉 You won ₹{prize} in Contest #{contest_id}!"
                cursor.execute("""
                    INSERT INTO notifications (user_id, message)
                    VALUES (%s, %s)
                """, (user_id, message))
